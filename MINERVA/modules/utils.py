...
def get_pred_dirs(o, joint_latent, mod_latent, impute, batch_correct, translate, input):
    ...
    for subset_id in range(len(o.s_joint)):
        ...
        if input:
            ...
            # dirs[subset_id]["x_downsample"] = {}
            for m in o.combs[subset_id]:
                dirs[subset_id]["x"][m] = pj(subset_dir, "x", m)
                # dirs[subset_id]["x_noise"][m] = pj(subset_dir, "x_noise", m)
                # dirs[subset_id]["x_downsample"][m] = pj(subset_dir, "x_downsample", m)
    return dirs
...
def random_noise(input, sigma = 0.2):

    noise = th.ones_like(input)
    for i in range(input.size(0)):
        # create the noise
        noise[i] = th.tensor(np.random.normal(0, sigma, input.size(1)))
    input += noise
    input[input < 0.] = 0.
    return input
...
def downsample(input):
    ...
    matrix = r['t'](r['as.matrix'](input.numpy()))
    row_names = StrVector([str(i) for i in range(1, input.shape[1] + 1)])
    col_names = StrVector([str(i) for i in range(1, input.shape[0] + 1)])
    setattr(matrix, 'dimnames', ListVector({'x': row_names, 'y': col_names}))
    downsampled = r['t'](r['downsampleMatrix'](matrix, prop = random.random() * 0.5))
    downsampled_array = np.array(r['as.matrix'](downsampled)).reshape(-1, r['dim'](matrix)[0])
    input = th.from_numpy(downsampled_array.astype(np.float32))

    return input


def fusion(param, mix1,mix2):
    input = mix1 * param[:, 2].unsqueeze(1) + mix2 * (1 - param[:, 2]).unsqueeze(1)
    return input


def fusion_data(o, mode, base_indirs, train_ratio):

    # get all batch filenames and paired mixup files
    all_files = {}
    all_mode = {"rna_adt":[], "rna":[], "adt":[]}
    file_num = {}

    for b in range(len(o.s)):
        # 3_C5270.csv
        flist = get_filenames(pj(base_indirs[b], mode[b][0]), "csv")
        all_files[b] = [str(b) + "_" + 
        ...
        if "rna" in folder_names:
            all_mode["rna"].append(b)
        if "adt" in folder_names:
            all_mode["adt"].append(b)
        if "rna" in folder_names and "adt" in folder_names:
            all_mode["rna_adt"].append(b)

    RNA_ADT, RNA, ADT = [], [], []
    for o in all_mode.keys():
        for b in all_mode[o]:
            if o == "rna_adt":
                RNA_ADT += all_files[b]
            if o == "rna":
    ...
    file_lists = {"rna_adt": RNA_ADT, "rna": RNA, "adt": ADT}
    file_pairs = {}
    for dataset_type, files in file_lists.items():
        shuffled_files = sorted(files, key = lambda x: random.random())
        file_pairs[dataset_type] = list(zip(files, shuffled_files))

    pair_mix = {}
    for s in range(0, len(mode)):
        if len(mode[s]) == 2:
            if mode[s][1] == "rna":
                Key = mode[s][1] + "_" + mode[s][0]
            else:
                Key = mode[s][0] + "_" + mode[s][1]
        else:
            Key = mode[s][0]
        num = random.sample(range(1, len(file_pairs[Key])), file_num[s])
        pair_mix[s] = [file_pairs[Key][n - 1] for n in num]
